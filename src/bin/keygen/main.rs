...
fn main() {
    ...
    let mut file = File::create(format!("{name}.pub")).unwrap_or_else(|_| {
        println!("failed to open output file for public key");
        exit(1);
    });

    match file.write_all(public_key.as_bytes()) {
        Ok(_) => {}
        Err(_) => {
    ...
    file = File::create(&name).unwrap_or_else(|_| {
        println!("failed to open output file for private key");
        exit(1);
    });

    match file.write_all(private_key.as_bytes()) {
        Ok(_) => {}
        Err(_) => {
